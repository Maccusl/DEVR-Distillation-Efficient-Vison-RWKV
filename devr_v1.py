...
#     return torch.cat((x_shortcut, output), dim=1)
...
#         self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 2, self.embed_dims))
...
#         #projector for ofa
#         self.proj = {}
#         self.proj[stages[0]] = create_proj(1)
#         self.proj[stages[1]] = create_proj(2)
#         self.proj[stages[2]] = create_proj(3)
...
#             x = layer(x, (P,P))
#         x = self.norm(x)
#         return x[:, 0], x[:, 1],dis_dict
    
#     def forward(self, x):
#         x, x_dist,dis_dict = self.forward_features(x)
#         x = self.head(x)
#         x_dist = self.head_dist(x_dist)
#         if self.training:
#             return (x, x_dist),dis_dict
#         else:
...
#                  depth=12)
#     if pretrained:
#         model.load_state_dict("vrwkv_tiny.pth")
#     return model